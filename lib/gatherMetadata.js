...
const checkAdditionalPropTypeData = (
  ...
  if (propMeta.source) {
    if (!propMeta.sourceConfigs) {
      throw new Error(
        `No source configs for prop '${propName}' ` +
        `of component '${componentName}'`
      );
    }
    
    propMeta.source.forEach(source => {
      if (!propMeta.sourceConfigs[source]) {
        throw new Error(
          `No '${source}' source config for prop '${propName}' ` +
          `of component '${componentName}'`
        );
      }
    });
